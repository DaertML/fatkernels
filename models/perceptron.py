...
def mlp(X, W1, B1, W2, B2):
    Y = torch.empty((BLOCK_SIZE, N), dtype=torch.float32, device='cuda')
...
if __name__ == "__main__":
    # Parameters
    M = 64  # Size of the first hidden layer
    ...
    X = torch.rand((BLOCK_SIZE, K), dtype=torch.float32, device=dev)
    W1 = torch.rand((K, M), dtype=torch.float32, device=dev)
    B1 = torch.rand((M,), dtype=torch.float32, device=dev)
    W2 = torch.rand((M, N), dtype=torch.float32, device=dev)
    B2 = torch.rand((N,), dtype=torch.float32, device=dev)

    res = mlp(X,W1,B1,W2,B2)
