...
@triton.jit
def leakyrelu_kernel(x_ptr, out_ptr, N: tl.constexpr, block_size: tl.constexpr):
    #pdb.set_trace()
...
def leakyrelu(x):
    ...
    leakyrelu_kernel[grid](x, out, N, BLOCK_SIZE)
...
if __name__ == "__main__":
    ...
    output_triton = leakyrelu(x)
    print(f"Triton result: {output_triton}")

    output_torch = torch.nn.functional.leaky_relu(x)
