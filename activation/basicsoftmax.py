...
if __name__ == "__main__":
    ...
    x = torch.tensor([1.0, -1.0, 0, 4.0], dtype=torch.float32)

    # Compute the relu layer using Triton kernel
    output_triton = softmax(x)
    print(f"Triton result: {output_triton}")

    output_torch = torch.nn.functional.softmax(x)

    print(f"Torch result: {output_torch}")
    print(f'The maximum difference between torch and triton is '
      f'{torch.max(torch.abs(output_torch - output_triton))}')

    print("Results dont match. Follows implementation of scratchml with numerical inestabilities. It is ok.")